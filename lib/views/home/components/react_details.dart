...
class ReactDetailsScreen extends StatefulWidget {
  final String? title;
  const ReactDetailsScreen({
    Key? key,
    this.title,
  }) : super(key: key);

  @override
  _ReactDetailsScreenState createState() => _ReactDetailsScreenState();
}

class _ReactDetailsScreenState extends State<ReactDetailsScreen> {
  int _selectedTag = 0;
    ...
    return Expanded(
      child: AnimationLimiter(
        child: ListView.separated(
          ...
          shrinkWrap: true,
          itemCount: reactLessonList.length,
          itemBuilder: (_, index) {
            return AnimationConfiguration.staggeredList(
              ...
              child: SlideAnimation(
                ...
                child: FadeInAnimation(
                  child: _ReactLessonCard(lesson: reactLessonList[index]),
                ),
              ),
            );
          },
...
class _ReactLessonCard extends StatelessWidget {
  final ReactLesson lesson;
  const _ReactLessonCard({Key? key, required this.lesson}) : super(key: key);
